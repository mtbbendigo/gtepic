...
class TitleTextPictureBlockController extends BlockController {
	
	protected $btName = 'Title Text and Picture';
	protected $btDescription = 'This block displays a Title, text and image. Use Templates to change the layout of Text and Picture';
	protected $btTable = 'btDCTitleTextPicture';
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('TitleTextPictureBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('TitleTextPictureBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('TitleTextPictureBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('TitleTextPictureBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('TitleTextPictureBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('TitleTextPictureBlockController', 'replaceDownloadFileID'),
			$text);
    ...
    public function getArticleTitle(){
        if(!empty($this->field_1_textbox_text))
        {
            return $this->field_1_textbox_text;
        }
        return '<h1>Empty Title</h1>';
    }

    public function getArticleImage()
    {
//        if(!empty($this->field_3_image_fID))
//        {
            return $this->get_image_object($this->field_3_image_fID, 0, 0, false);
//        }
//        else if(!empty($this->field_4_image_fID))
//        {
//            return get_image_object($this->field_4_image_fID, 0, 0, false);
//        }
//        return '<h1>Empty Image</h1>';
    }

    public function getArticleContent()
    {
        if(!empty($this->field_2_wysiwyg_content))
        {
