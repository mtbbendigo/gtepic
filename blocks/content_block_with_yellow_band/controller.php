...
class ContentBlockWithYellowBandBlockController extends BlockController {
	
	protected $btName = 'Content Block With Yellow Band';
	protected $btDescription = 'Displays text and an image and a hyperlink enclosed in a yellow band at the bottom. The layout of the text and image can be changed by applying a custom template.';
	protected $btTable = 'btDCContentBlockWithYellowBand';
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('ContentBlockWithYellowBandBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('ContentBlockWithYellowBandBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('ContentBlockWithYellowBandBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('ContentBlockWithYellowBandBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('ContentBlockWithYellowBandBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('ContentBlockWithYellowBandBlockController', 'replaceDownloadFileID'),
			$text);
