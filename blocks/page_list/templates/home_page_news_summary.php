...
<?php foreach ($pages as $page):
     ...
     */
          ...
          $summaryTitle = '';
            $image = null;
        $newsBlockTypes = array('title_text_picture');
          $pageBlocks = $page->getBlocks('Add to News');
          if (count($pageBlocks) > 0) {
              foreach ($pageBlocks as $pb) {
                  if ($pb->btHandle == $newsBlockTypes[0]) {
                      $summaryTitle = $pb->getController()->getArticleTitle();
                      $image = $pb->getController()->getArticleImage();
                      $excerpt = $pb->getController()->getArticleContent(); //NOTE: getContent() function is specific to the content block type -- it cannot be called on just any kind of block!
          	           if ($controller->truncateSummaries) {
                ...
                <div class="content-container">
                    <div class="row">
                        <div class="medium-3 large-3 columns">
                            ...
                            <img src="<?php echo $image->src?>" width="<?php echo $image->width ?>" height="<?php echo $image->height ?>" style="border: #fff solid 1px;">
                            </div>
                        </div>
                        <div class="medium-9 large-9 columns">
                            <div class="pageList-article">
                                <h3>
                                    <a href="<?php echo $url ?>" class="page-list-title" target="<?php echo $target ?>"><?php echo $summaryTitle ?></a>
                                </h3>
                                <div class="postedDate"><?php echo strtoupper($date);?></div>
                                <div class="newsExcerpt show-for-medium-up"><p><?php echo $excerpt?></p></div>
                                <span class="read-more"><b>&raquo;</b>&nbsp;<a href="<?php  echo $url; ?>"><?php  echo t('READ MORE'); ?> </a></span>
                            </div>
                        </div>
                    </div>
                </div>
...
<?php if ($showRss): ?>
    <div class="homePageNewsEntryLink">
